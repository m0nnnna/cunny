...
export function RoomTimeline({ room, eventId, roomInputRef, editor }: RoomTimelineProps) {
  ...
  const effectiveMessageSpacing = ircMode ? '0' : messageSpacing;
  const effectiveMessageLayout = ircMode ? MessageLayout.Compact : messageLayout;
  ...
  >(
    {
      [MessageEvent.RoomMessage]: (mEventId, mEvent, item, timelineSet, collapse) => {
        ...
        return (
          <Message
            ...
            room={room}
            mEvent={mEvent}
            messageSpacing={effectiveMessageSpacing}
            messageLayout={effectiveMessageLayout}
            ircMode={ircMode}
            collapse={collapse}
          ...
          >
            {mEvent.isRedacted() ? (
              ...
              <RenderMessageContent
                ...
                outlineAttachment={effectiveMessageLayout === MessageLayout.Bubble}
              />
            )}
          </Message>
        );
      ...
      [StateEvent.RoomMember]: (mEventId, mEvent, item) => {
        ...
        const timeJSX = (
          <Time
            ts={mEvent.getTs()}
            compact={effectiveMessageLayout === MessageLayout.Compact}
            hour24Clock={hour24Clock}
            dateFormatString={dateFormatString}
          />
        );
  ...
  const eventRenderer = (item: number) => {
    ...
    const collapsed =
      ...
      (ircMode || minuteDifference(prevEvent.getTs(), mEvent.getTs()) < 2);
    ...
    const newDividerJSX =
      newDivider && eventJSX && eventSender !== mx.getUserId() ? (
        <MessageBase space={effectiveMessageSpacing}>
      ...
      <Scroll ref={scrollRef} visibility="Hover">
        ...
        >
            ...
            <div
              style={{
                padding: `${config.space.S700} ${config.space.S400} ${config.space.S600} ${
                  effectiveMessageLayout === MessageLayout.Compact ? config.space.S400 : toRem(64)
                }`,
              }}
            >
              <RoomIntro room={room} />
          ...
          {(canPaginateBack || !rangeAtStart) &&
            (effectiveMessageLayout === MessageLayout.Compact ? (
              <>
                <MessageBase>
